...
def encrypt():
	# проверка наличия 2 одинаковых идущих подряд букв и чётности длины текста
	temp_orig=[]
	for x in range(len(orig)):
		if x!=len(orig)-1 and orig[x]==orig[x+1]:
			temp_orig.append(orig[x])
			temp_orig.append(fixsym)
		else:
			temp_orig.append(orig[x])
	if len(temp_orig)%2!=0:
		temp_orig.append(fixsym)
	# разделение текста на биграммы
	bigramms=[]
	c=0
	for i in range (0,len(temp_orig),2):
		bigramm=[]
		bigramm.append(temp_orig[c])
		bigramm.append(temp_orig[c+1])
